...
class HarnessService : Service() {
    ...
    override fun onCreate() {
        ...
        startForeground(NOTIFICATION_ID, buildNotification(statusText(RuntimeManager.state.value.phase)))
        observeState()
    }
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        getSystemService(NotificationManager::class.java)
            .notify(NOTIFICATION_ID, buildNotification(statusText(RuntimeManager.state.value.phase)))
        return START_STICKY
    }
    ...
    private fun observeState() {
        scope.launch {
            RuntimeManager.state.map { it.phase }.distinctUntilChanged().collect { phase ->
                getSystemService(NotificationManager::class.java)
                    .notify(NOTIFICATION_ID, buildNotification(statusText(phase)))
            }
        }
    }
